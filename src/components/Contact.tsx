...
export default function Contact() {
  ...
  const contactItems = [
    ...
    },
    {
      ...
      content: <span className="font-semibold text-text">{t.contact.addressValue}</span>,
    },
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4 reveal">
            {contactItems.map((item, i) => (
              <div key={i} className="flex items-start gap-4 bg-white rounded-xl p-5 border border-border">
                ...
                <div>
                  <div className="text-xs text-text-subtle font-medium uppercase tracking-wider mb-1">{item.label}</div>
                  {item.content}
                </div>
              </div>
            ))}
